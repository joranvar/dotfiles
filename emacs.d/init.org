...
#+BEGIN_SRC emacs-lisp
  ...
  (defun joranvar/compilation-notify (buffer reason)
    ...
    (let ((exit-status (or
      ...
      (if (> exit-status 0)
          (progn
            (joranvar/play-sound "/home/joranvar/Downloads/buildWarning.wav")
            (alert (format "Compilation finished with errors (exit-status %d)" exit-status)
                   :buffer buffer :severity 'high))
        (joranvar/play-sound "/home/joranvar/Downloads/buildSuccess.wav")
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun joranvar/change-volume (amount)
    ...
    (interactive)
    (let ((volume-string (format "%d%%%s" (abs amount) (if (< 0 amount) "+" "-"))))
      (start-process-shell-command (format "amixer sset Master %s" volume-string) nil (format "amixer sset Master %s" volume-string))
      (message "%s" (shell-command-to-string "amixer sget Master | grep t: | cat"))))

  (exwm-input-set-key (kbd "<XF86AudioRaiseVolume>") (joranvar/change-volume 5))
  (exwm-input-set-key (kbd "<XF86AudioLowerVolume>") (joranvar/change-volume -5))
